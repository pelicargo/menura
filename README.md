...
Copy `.env.example` and `agents.json.example` to `.env` and `agents.json`, respectively, and modify the env as necessary. `agents.json` can be generated by navigating to http(s)://(domain)/admin/manager and configure as necessary. The username is `menura` Make sure to test your configuration and then export it, and use the generated file for your `agents.json`.
